...
1. Store secrets in Vault (e.g. KV v2) and expose them as env vars via:
   ...
   - **Kubernetes External Secrets Operator**: create `ExternalSecret` resources that sync Vault KV to Kubernetes `Secret`s; mount or envFrom into NeuronDB/NeuronAgent/NeuronDesktop/NeuronMCP pods (deploy Agent, Desktop, MCP from their respective repos)
