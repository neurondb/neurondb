...
-- Alias: cityblock is another name for L1 distance
CREATE FUNCTION vector_cityblock_distance(vector, vector) RETURNS real
    AS $$ SELECT vector_l1_distance($1, $2); $$
    LANGUAGE SQL IMMUTABLE STRICT PARALLEL SAFE;
COMMENT ON FUNCTION vector_cityblock_distance IS 'Cityblock distance (alias for L1/Manhattan distance)';
...
-- Additional distance metrics implemented as PL/pgSQL wrappers
-- Bray-Curtis distance: sum(|a_i - b_i|) / sum(|a_i + b_i|)
CREATE FUNCTION vector_bray_curtis_distance(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
    diff_sum double precision := 0.0;
    sum_sum double precision := 0.0;
    i integer;
BEGIN
    a_arr := vector_to_array($1);
    b_arr := vector_to_array($2);
    IF array_length(a_arr, 1) != array_length(b_arr, 1) THEN
        RAISE EXCEPTION 'vector dimensions must match';
    END IF;
    FOR i IN 1..array_length(a_arr, 1) LOOP
        diff_sum := diff_sum + ABS(a_arr[i] - b_arr[i]);
        sum_sum := sum_sum + ABS(a_arr[i] + b_arr[i]);
    END LOOP;
    IF sum_sum = 0.0 THEN
        RETURN 0.0;
    END IF;
    RETURN diff_sum / sum_sum;
END;
$$;
COMMENT ON FUNCTION vector_bray_curtis_distance IS 'Bray-Curtis distance: sum(|a_i - b_i|) / sum(|a_i + b_i|)';

-- Canberra distance: sum(|a_i - b_i| / (|a_i| + |b_i|))
CREATE FUNCTION vector_canberra_distance(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
...
BEGIN
    ...
    END IF;
    FOR i IN 1..array_length(a_arr, 1) LOOP
        denom := ABS(a_arr[i]) + ABS(b_arr[i]);
        IF denom > 0.0 THEN
            dist := dist + ABS(a_arr[i] - b_arr[i]) / denom;
        END IF;
    END LOOP;
    RETURN dist;
END;
$$;
COMMENT ON FUNCTION vector_canberra_distance IS 'Canberra distance: sum(|a_i - b_i| / (|a_i| + |b_i|))';

-- Sokal-Michener distance: (M01 + M10) / (M00 + M01 + M10 + M11) for binary vectors
-- For continuous vectors, treat as binary: 1 if > 0, 0 otherwise
CREATE FUNCTION vector_sokal_michener_distance(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
...
BEGIN
    ...
    END IF;
    FOR i IN 1..array_length(a_arr, 1) LOOP
        a_bit := CASE WHEN a_arr[i] > 0 THEN 1 ELSE 0 END;
        b_bit := CASE WHEN b_arr[i] > 0 THEN 1 ELSE 0 END;
        IF a_bit = 0 AND b_bit = 0 THEN
            m00 := m00 + 1;
        ELSIF a_bit = 0 AND b_bit = 1 THEN
            m01 := m01 + 1;
        ELSIF a_bit = 1 AND b_bit = 0 THEN
            m10 := m10 + 1;
        ELSE
            m11 := m11 + 1;
        END IF;
    END LOOP;
    IF (m00 + m01 + m10 + m11) = 0 THEN
        RETURN 0.0;
    END IF;
    RETURN (m01 + m10)::double precision / (m00 + m01 + m10 + m11)::double precision;
END;
$$;
COMMENT ON FUNCTION vector_sokal_michener_distance IS 'Sokal-Michener distance for binary vectors';

-- Rogers-Tanimoto distance: 2 * (M01 + M10) / (2 * (M01 + M10) + M00 + M11)
CREATE FUNCTION vector_rogers_tanimoto_distance(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
...
BEGIN
    ...
    FOR i IN 1..array_length(a_arr, 1) LOOP
        ...
        END IF;
    END LOOP;
    IF (2 * (m01 + m10) + m00 + m11) = 0 THEN
        RETURN 0.0;
    END IF;
    RETURN (2.0 * (m01 + m10)) / (2.0 * (m01 + m10) + m00 + m11);
END;
$$;
COMMENT ON FUNCTION vector_rogers_tanimoto_distance IS 'Rogers-Tanimoto distance for binary vectors';

-- Russell-Rao distance: (M00 + M01 + M10) / (M00 + M01 + M10 + M11)
CREATE FUNCTION vector_russell_rao_distance(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
...
BEGIN
    ...
    IF (m00 + m01 + m10 + m11) = 0 THEN
        RETURN 0.0;
    END IF;
    RETURN (m00 + m01 + m10)::double precision / (m00 + m01 + m10 + m11)::double precision;
END;
$$;
COMMENT ON FUNCTION vector_russell_rao_distance IS 'Russell-Rao distance for binary vectors';

-- Matching coefficient: (M00 + M11) / (M00 + M01 + M10 + M11) for binary vectors
CREATE FUNCTION vector_matching_coefficient(vector, vector) RETURNS double precision
    LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
DECLARE
    a_arr real[];
    b_arr real[];
...
BEGIN
    ...
    IF (m00 + m01 + m10 + m11) = 0 THEN
        RETURN 0.0;
    END IF;
    RETURN (m00 + m11)::double precision / (m00 + m01 + m10 + m11)::double precision;
END;
$$;
COMMENT ON FUNCTION vector_matching_coefficient IS 'Matching coefficient for binary vectors: (M00 + M11) / total';
...
CREATE FUNCTION build_knn_graph(text, text, integer)
    RETURNS real[][]
    AS 'MODULE_PATHNAME', 'build_knn_graph'
    LANGUAGE C STABLE STRICT;
COMMENT ON FUNCTION build_knn_graph IS 'Build KNN graph: (table, vector_col, k) returns 2D array of [node_id, neighbor_id, distance]';

CREATE FUNCTION compute_embedding_quality(text, text, text)
    RETURNS real
    AS 'MODULE_PATHNAME', 'compute_embedding_quality'
    LANGUAGE C STABLE STRICT;
COMMENT ON FUNCTION compute_embedding_quality IS 'Compute embedding quality: (table, vector_col, cluster_col) returns silhouette score';
...
-- Reciprocal Rank Fusion Table Wrapper
...
-- Ensemble Reranking Table Wrappers
...
-- Drift Detection Table Wrappers
...
BEGIN
    ...
    FOR window_rec IN EXECUTE sql_text LOOP
        -- Calculate centroid for baseline (first window) using lateral join
        sql_text := format('SELECT array_to_vector(array_agg(avg_val ORDER BY dim))::vector 
                           FROM (
                               SELECT dim, AVG(val) as avg_val
                               FROM %I, LATERAL unnest(vector_to_array(%I)) WITH ORDINALITY AS t(val, dim)
                          ...
                          table_name, vector_column, timestamp_column, window_rec.window_start);
        EXECUTE sql_text INTO baseline_centroid_vec;
        
        -- Calculate centroid for current window using lateral join
        sql_text := format('SELECT array_to_vector(array_agg(avg_val ORDER BY dim))::vector 
                           FROM (
                               SELECT dim, AVG(val) as avg_val
                               FROM %I, LATERAL unnest(vector_to_array(%I)) WITH ORDINALITY AS t(val, dim)
...
BEGIN
	...
	CASE lower(algorithm)
		...
		WHEN 'titans_llm' THEN
			-- Delegate to Titans LLM training
			RETURN neurondb.train('default', 'titans_llm', table_name, label_col, ARRAY[feature_col], params);
		ELSE
...
-- Titans LLM Training Function
CREATE OR REPLACE FUNCTION neurondb_train_titans_llm(
	project_name text,
	table_name text,
	target_column text DEFAULT NULL,
	feature_columns text[] DEFAULT ARRAY[]::text[],
...
LANGUAGE c
AS 'MODULE_PATHNAME', 'neurondb_train_titans_llm';
COMMENT ON FUNCTION neurondb_train_titans_llm IS 'Train a Titans (Learning to Memorize at Test Time) LLM model';
...
BEGIN
	...
	CASE algo
		...
		WHEN 'transformer_llm', 'custom_llm', 'titans_llm' THEN
			-- Transformer/Titans LLM prediction via ONNX Runtime or server
...
LANGUAGE sql STABLE AS $$
	SELECT * FROM (VALUES
		...
		('transformer_llm', 'llm', true, 'Custom transformer LLM', '{"epochs": 5, "d_model": 256}'::jsonb),
		('titans_llm', 'llm', true, 'Titans LLM - Learning to Memorize at Test Time', '{"variant": "mac", "memory_depth": 2, "epochs": 5}'::jsonb)
...
DO $$
BEGIN
    -- Add reranking columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'neurondb' AND table_name = 'rag_pipelines' AND column_name = 'rerank_enabled') THEN
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN rerank_enabled BOOLEAN DEFAULT false;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN rerank_top_k INTEGER DEFAULT 10;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN rerank_initial_k INTEGER DEFAULT 50;
    END IF;
    
    -- Add hybrid search columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'neurondb' AND table_name = 'rag_pipelines' AND column_name = 'hybrid_enabled') THEN
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN hybrid_enabled BOOLEAN DEFAULT false;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN vector_weight FLOAT DEFAULT 0.7;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN text_weight FLOAT DEFAULT 0.3;
    END IF;
    
    -- Add evaluation columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'neurondb' AND table_name = 'rag_pipelines' AND column_name = 'evaluation_enabled') THEN
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN evaluation_enabled BOOLEAN DEFAULT false;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN evaluation_metrics TEXT[] DEFAULT ARRAY['relevancy', 'semantic_similarity'];
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN evaluation_threshold FLOAT DEFAULT 0.7;
    END IF;
    
    -- Add LLM generation columns
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'neurondb' AND table_name = 'rag_pipelines' AND column_name = 'llm_model') THEN
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN llm_model TEXT DEFAULT 'gpt-3.5-turbo';
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN llm_temperature FLOAT DEFAULT 0.7;
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN llm_max_tokens INTEGER DEFAULT 500;
    END IF;
    
    -- Add updated_at column
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'neurondb' AND table_name = 'rag_pipelines' AND column_name = 'updated_at') THEN
        ALTER TABLE neurondb.rag_pipelines ADD COLUMN updated_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP;
    END IF;
END $$;
...
-- =============================================================================
-- Additional ML Function Wrappers for Table-based Interfaces
-- =============================================================================

-- K-means clustering wrapper
CREATE FUNCTION neurondb.cluster_kmeans(
    table_name text,
    vector_column text,
    k integer,
    max_iterations integer DEFAULT 100
) RETURNS TABLE(id integer, cluster_id integer)
LANGUAGE plpgsql VOLATILE AS $$
DECLARE
    assignments integer[];
    sql_text text;
    ...
    i integer;
BEGIN
    SELECT cluster_kmeans(table_name, vector_column, k, max_iterations) INTO assignments;
    
    IF assignments IS NOT NULL THEN
        sql_text := format('SELECT row_number() OVER ()::integer FROM %I ORDER BY ctid', table_name);
        i := 1;
        FOR row_id IN EXECUTE sql_text LOOP
            IF i <= array_length(assignments, 1) THEN
                id := row_id;
...
END;
$$;
COMMENT ON FUNCTION neurondb.cluster_kmeans IS 'K-means clustering with table-based interface';

-- PCA dimensionality reduction wrapper
CREATE FUNCTION neurondb.reduce_pca(
    table_name text,
    vector_column text,
    target_dim integer
) RETURNS TABLE(id integer, reduced_vector vector)
LANGUAGE plpgsql VOLATILE AS $$
DECLARE
    reduced_matrix real[][];
    reduced_row real[];
    ...
    row_size integer;
BEGIN
    SELECT reduce_pca(table_name, vector_column, target_dim) INTO reduced_matrix;
    
    IF reduced_matrix IS NOT NULL THEN
        dim_count := array_length(reduced_matrix, 1);
        row_size := array_length(reduced_matrix, 2);
        IF dim_count IS NOT NULL AND dim_count > 0 AND row_size IS NOT NULL THEN
            -- Get row IDs from the table
            sql_text := format('SELECT row_number() OVER ()::integer FROM %I ORDER BY ctid', table_name);
            i := 1;
            FOR row_id IN EXECUTE sql_text LOOP
                IF i <= dim_count THEN
                    -- Extract row i from 2D array by unnesting and grouping
                    sql_text := format('
                        WITH arr AS (SELECT $1::real[][] as matrix),
                        unnested AS (
                            SELECT 
                                ((row_number() OVER () - 1) / $2::integer) + 1 as row_num,
                                val,
                                ord
                            FROM arr, LATERAL unnest(matrix) WITH ORDINALITY AS t(val, ord)
                        )
                        SELECT array_agg(val ORDER BY ord)::real[]
                        FROM unnested
                        WHERE row_num = $3
                        GROUP BY row_num', i);
                    EXECUTE sql_text USING reduced_matrix, row_size, i INTO reduced_row;
                    IF reduced_row IS NOT NULL AND array_length(reduced_row, 1) > 0 THEN
                        id := row_id;
                        reduced_vector := array_to_vector(reduced_row)::vector;
                        RETURN NEXT;
                    END IF;
                END IF;
                i := i + 1;
...
END;
$$;
COMMENT ON FUNCTION neurondb.reduce_pca IS 'PCA dimensionality reduction with table-based interface';

-- Product Quantization codebook training wrapper
-- Note: train_pq_codebook returns bytea, so this is a simplified wrapper
CREATE FUNCTION neurondb.train_pq_codebook(
    table_name text,
    vector_column text,
...
END;
$$;
COMMENT ON FUNCTION neurondb.train_pq_codebook IS 'Product Quantization codebook training with table-based interface (simplified)';

-- Outlier detection wrapper
CREATE FUNCTION neurondb.detect_outliers_zscore(
    table_name text,
    vector_column text,
    threshold double precision DEFAULT 3.0,
    method text DEFAULT 'zscore'
) RETURNS TABLE(id integer, is_outlier boolean, z_score double precision)
LANGUAGE plpgsql VOLATILE AS $$
DECLARE
    outlier_flags boolean[];
    sql_text text;
    ...
    i integer;
BEGIN
    SELECT detect_outliers_zscore(table_name, vector_column, threshold, method) INTO outlier_flags;
    
    IF outlier_flags IS NOT NULL THEN
        sql_text := format('SELECT row_number() OVER ()::integer FROM %I ORDER BY ctid', table_name);
        i := 1;
        FOR row_id IN EXECUTE sql_text LOOP
            IF i <= array_length(outlier_flags, 1) THEN
                id := row_id;
                is_outlier := COALESCE(outlier_flags[i], false);
                z_score := CASE WHEN COALESCE(outlier_flags[i], false) THEN threshold ELSE 0.0 END;
                RETURN NEXT;
            END IF;
            i := i + 1;
        END LOOP;
