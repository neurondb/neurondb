...
#
# Runs NeuronAgent server natively or in Docker mode.
...
NEURONAGENT_DIR="${PROJECT_ROOT}/NeuronAgent"
...
run_docker_mode() {
    local service_name="neuronagent"
    
    print_info "Running NeuronAgent in Docker mode"
    check_docker
...
check_postgresql_connection() {
    ...
    local database="${DB_NAME:-neurondb}"
...
build_binary() {
    ...
    # Try direct go build
    if [ -f "cmd/agent-server/main.go" ]; then
        mkdir -p bin
        if go build -o "bin/$binary_name" "./cmd/agent-server/main.go" 2>/dev/null; then
            print_success "Binary built successfully"
            return 0
        fi
    fi
...
find_binary() {
    ...
    local possible_paths=(
        "$project_dir/bin/neuronagent"
        "$project_dir/bin/$binary_name"
        "$project_dir/agent-server"
        "$project_dir/bin/agent-server"
    )
...
setup_environment() {
    ...
    print_debug "  SERVER_HOST=$SERVER_HOST"
...
run_native_mode() {
    print_info "Running NeuronAgent in native mode"
    ...
    if [ -d "$NEURONAGENT_DIR" ]; then
        install_go_deps "$NEURONAGENT_DIR"
        
        # Check for Python requirements (optional)
        if [ -f "$NEURONAGENT_DIR/requirements.txt" ]; then
            install_python_deps "$NEURONAGENT_DIR/requirements.txt"
        fi
    else
        error_exit "NeuronAgent directory not found: $NEURONAGENT_DIR"
    fi
    
    # Find or build binary
    local binary_path
    if binary_path=$(find_binary "$NEURONAGENT_DIR" "neuronagent"); then
        print_success "Found binary: $binary_path"
    else
        print_info "Binary not found, attempting to build..."
        if build_binary "$NEURONAGENT_DIR" "neuronagent"; then
            if binary_path=$(find_binary "$NEURONAGENT_DIR" "neuronagent"); then
            ...
            if [ -f "$NEURONAGENT_DIR/cmd/agent-server/main.go" ]; then
                print_info "Using 'go run' as fallback"
                binary_path=""
            else
                error_exit "Binary not found and build failed. Please build manually: cd $NEURONAGENT_DIR && make build"
            fi
        fi
    fi
    ...
    echo ""
    print_info "=========================================="
    print_success "Starting NeuronAgent Server"
    print_info "=========================================="
    if [ -n "$binary_path" ]; then
        print_info "Binary: $binary_path"
    else
        print_info "Running with: go run cmd/agent-server/main.go"
    fi
...
show_help() {
    cat << EOF
$SCRIPT_NAME - NeuronAgent Run Script
