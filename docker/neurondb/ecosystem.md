...
Complete Docker deployment guide for running NeuronDB with NeuronAgent and NeuronMCP. **NeuronAgent** and **NeuronMCP** are in separate repositories (**neuron-agent**, **neuron-mcp**). Run NeuronDB from this repo; run Agent and MCP from their repos, all connecting to the same NeuronDB PostgreSQL instance.
...
Navigate to **neuron-agent** repo (clone as sibling or set NEURON_AGENT_REPO):

```bash
cd "${NEURON_AGENT_REPO:-../neuron-agent}/docker"
# or: cd /path/to/neuron-agent/docker
```
...
See the **neuron-agent** repo for Docker configuration options.
...
See the **neuron-mcp** repo for Docker configuration options.
...
- **NeuronAgent Docker**: neuron-agent repo
- **NeuronMCP Docker**: neuron-mcp repo
